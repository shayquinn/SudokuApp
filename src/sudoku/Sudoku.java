...
public class Sudoku {
	...
	private final int MATRIX_SIZE = (RANK * RANK); // the puzzle matrix is 9x9 for a rank of 3
	private final int FIRST_COL_ROW_NUMBER = 0; // zero based = 0; 1 based = 1
        
        int[][] solution;

    public Sudoku(String puzz) {
       solution = createSolution(puzz);
    }
    
    // return the solution as a string
	public String getSolutionString() {
		return matrix2String(solution);
                
	}
        
        		// return the matrix as a string of 81 digits 0-9
	// in typewriter order (left to right, top to bottom)
	private String matrix2String( int[][] matrix ) {
		StringBuilder strB = new StringBuilder( MATRIX_SIZE * MATRIX_SIZE );
		for(int row = 0; row < MATRIX_SIZE; row++) {
			for(int col = 0; col < MATRIX_SIZE; col++) {
				strB.append( new Integer( matrix[row][col] ).toString() );
			}
		}
		return strB.toString();
	}
        
        	// convert a string with 81 characters to a 2 dimensional array
	private int[][] createSolution(String p) {
		int[][] puzz = new int[MATRIX_SIZE][MATRIX_SIZE];
		int row = 0;
		int col = 0;
		for(int i = 0; i < p.length(); i++) {
                    row = (int)(i / MATRIX_SIZE);
                    if(col >= MATRIX_SIZE) col = FIRST_COL_ROW_NUMBER;
			puzz[row][col++] = Character.getNumericValue( p.charAt(i) );
		}
                solve(0,0,puzz);   // solves in place
